<div>
    <div class="tw-w-full tw-text-center">
        <button class="btn btn-primary" wire:click="downloadPdf"><span class="fas fa-download"></span>&nbsp; Dokumen Penetapan</button>
    </div>
    <div id="accordion">
        <div class="card tw-px-0">
            <div class="card-header tw-py-0 collapsible" href="#collapseOneReviu" data-toggle="collapse"
                data-target="#collapseOneReviu" aria-expanded="true" id="headingOne" aria-controls="collapseOneReviu"
                wire:ignore>
            ...
            <div id="collapseOneReviu" class="collapse show" aria-labelledby="headingOne" wire:ignore.self>
                <div class="card-body px-0 tw-pt-0">
                    <div class="table-responsive table-scroll" id='penetapan-table'>
                        <table class="table table-bordered datatable-sortable table-sm tw-text-sm table-complex">
                            ...
                            @foreach ($this->rencanaKinerjaUtama as $rencanaKinerja)
                                @if ($this->data['terkait'][$rencanaKinerja->id])
                                    @include('livewire.skp-guru.guru.tables.penetapan')
                                @endif
                            @endforeach
                        </table>
                    </div>
    ...
    @if ($this->skpGuru->status == 'draft')
        <div class="tw-w-full tw-text-center">
            <button type='button' class="btn btn-primary tw-text-cent3er" wire:click='confirmSKP'>Konfirmasi</button>
        </div>
    @endif
</div>
