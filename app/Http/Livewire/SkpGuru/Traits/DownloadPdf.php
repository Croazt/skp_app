...
trait DownloadPdf
{
    ...
    public function downloadSampul()
    {
        $view = (view('livewire.skp-guru.pdf.skp-guru-sampul', [
            'skpGuru' => $this->skpGuru,
        ])->render());
        $fileName =  \Carbon\Carbon::parse($this->skpGuru->skp->periode_awal)->year . '-' . $this->skpGuru->user_nip  . '-sampul.pdf';
        $margin = [
            'top' => 0,
            'bottom' => 0,
            'right' => 0,
        ...
        $saveToFile = storage_path('app/public/file/sampul/' . $fileName);
        Browsershot::html($view)
            ->setNodeBinary('/home/fachry/.nvm/versions/node/v18.12.1/bin/node')
            ->paperSize(210, 297)
            ->setNpmBinary('/home/fachry/.nvm/versions/node/v18.12.1/bin/npm')->savePdf($saveToFile);

        return response()->streamDownload(function ()  use ($fileName) {
            echo file_get_contents(env('APP_URL') . '/storage/file/sampul/' . $fileName);
        }, $fileName);
    }
    public function downloadAllPdf()
    {
        ...
        $zip = new ZipArchive();
        $zipName = 'skp-' . \Carbon\Carbon::parse($this->skpGuru->skp->periode_awal)->year . '-' . $this->skpGuru->user_nip . '.zip';
